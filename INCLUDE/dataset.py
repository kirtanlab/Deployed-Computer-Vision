...
class KeypointsDataset(data.Dataset):
    ...
    def interpolate(self, arr):
        ...
        arr_x = arr[:, :, 0] #x-coordinate is seperated
        arr_x = pd.DataFrame(arr_x) #creatting a dataframe
        arr_x = arr_x.interpolate(method="linear", limit_direction="both").to_numpy() #handling missing values using linear interpolation

        arr_y = arr[:, :, 1] #same for y coordinate
        ...
        if np.count_nonzero(~np.isnan(arr_x)) == 0:
            arr_x = np.zeros(arr_x.shape) #if there are Nan than it will set 0
